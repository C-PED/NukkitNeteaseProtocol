...
public class EventSerializer_v471 extends EventSerializer_v389 {
   ...
   protected EventSerializer_v471() {
      ...
      this.readers.put(EventDataType.STRIDER_RIDDEN_IN_LAVA_IN_OVERWORLD, (BiFunction)(b, h) -> StriderRiddenInLavaInOverworldEventData.INSTANCE);
      this.writers.put(EventDataType.STRIDER_RIDDEN_IN_LAVA_IN_OVERWORLD, (TriConsumer)(b, h, e) -> {
      });
      this.readers.put(EventDataType.SNEAK_CLOSE_TO_SCULK_SENSOR, (BiFunction)(b, h) -> SneakCloseToSculkSensorEventData.INSTANCE);
      this.writers.put(EventDataType.SNEAK_CLOSE_TO_SCULK_SENSOR, (TriConsumer)(b, h, e) -> {
      });
   }
   ...
   protected PiglinBarterEventData readPiglinBarter(ByteBuf buffer, BedrockCodecHelper helper) {
      ...
      ItemDefinition itemDefinition = (ItemDefinition)helper.getItemDefinitions().getDefinition(runtimeId);
   ...
   protected CopperWaxedOrUnwaxedEventData readCopperWaxedUnwaxed(ByteBuf buffer, BedrockCodecHelper helper) {
      int runtimeId = VarInts.readInt(buffer);
      BlockDefinition blockDefinition = (BlockDefinition)helper.getBlockDefinitions().getDefinition(runtimeId);
