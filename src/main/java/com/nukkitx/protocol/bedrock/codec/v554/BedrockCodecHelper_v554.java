...
public class BedrockCodecHelper_v554 extends BedrockCodecHelper_v534 {
   ...
   public ItemStackRequest readItemStackRequest(ByteBuf buffer) {
      ...
      TextProcessingEventOrigin origin = originVal == -1 ? null : (TextProcessingEventOrigin)this.textProcessingEventOrigins.getType(originVal);
      return new ItemStackRequest(requestId, (ItemStackRequestAction[])actions.toArray(new ItemStackRequestAction[0]), (String[])filteredStrings.toArray(new String[0]), origin);
   }
   ...
   protected ItemDescriptor readItemDescriptor(ByteBuf buffer, ItemDescriptorType type) {
      ...
      switch (type) {
         case DEFAULT:
            ...
            ItemDefinition definition = itemId == 0 ? ItemDefinition.AIR : (ItemDefinition)this.getItemDefinitions().getDefinition(itemId);
