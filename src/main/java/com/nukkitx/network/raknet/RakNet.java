...
public abstract class RakNet implements AutoCloseable {
   ...
   RakNet(EventLoopGroup eventLoopGroup, EventLoopGroup workerGroup) {
      this.bootstrap = (Bootstrap)((Bootstrap)((Bootstrap)(new Bootstrap()).option(ChannelOption.ALLOCATOR, ByteBufAllocator.DEFAULT)).option(ChannelOption.SO_RCVBUF, 2097152)).option(ChannelOption.SO_SNDBUF, 2097152);
   ...
   public CompletableFuture<Void> bind() {
      ...
      future.whenComplete((aVoid, throwable) -> {
         if (throwable != null) {
            ...
            this.tickFuture = this.nextEventLoop().scheduleAtFixedRate(this::onTick, 0L, 10L, TimeUnit.MILLISECONDS);
         }
      });
      return future;
   }
