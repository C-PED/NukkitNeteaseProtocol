...
public class BitQueue {
   ...
   public void add(boolean bit) {
      ...
      byte[] var10000 = this.queue;
      var10000[by] ^= (byte)(((bit ? 255 : 0) ^ this.queue[by]) & bi);
   ...
   private void resize(int capacity) {
      ...
      if ((this.tail & 7) == 0) {
         ...
         for(int cursor = 0; cursor < size; by2 = by2 + 1 & this.queue.length - 1) {
            int mask = (1 << tailBits) - 1 & 255;
            int bit1 = (this.queue[tailIdx] & ~mask & 255) >>> tailBits;
            int bit2 = this.queue[by2] << 8 - tailBits;
   ...
   public int size() {
      if (this.head > this.tail) {
         ...
         return this.head < this.tail ? (this.queue.length << 3) - (this.tail - this.head) : 0;
      }
   }
   ...
   public void set(int n, boolean bit) {
      if (n < this.size() && n >= 0) {
         ...
         byte[] var10000 = this.queue;
         var10000[arrIdx] ^= (byte)(((bit ? 255 : 0) ^ this.queue[arrIdx]) & mask);
      }
   }
