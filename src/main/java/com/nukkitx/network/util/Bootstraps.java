...
public final class Bootstraps {
   ...
   public static void setupBootstrap(Bootstrap bootstrap, boolean datagram) {
      Class<? extends Channel> channel = datagram ? EventLoops.getChannelType().getDatagramChannel() : EventLoops.getChannelType().getSocketChannel();
      bootstrap.channel(channel);
      setupAbstractBootstrap(bootstrap);
   }

   public static void setupServerBootstrap(ServerBootstrap bootstrap) {
      Class<? extends ServerSocketChannel> channel = EventLoops.getChannelType().getServerSocketChannel();
      bootstrap.channel(channel);
      setupAbstractBootstrap(bootstrap);
   }
   ...
   private static boolean checkVersion(int[] ver, int i) {
      if (ver[i] > REUSEPORT_VERSION[i]) {
         ...
         return ver.length == i + 1 ? true : checkVersion(ver, i + 1);
      } else {
         return false;
      }
   }
